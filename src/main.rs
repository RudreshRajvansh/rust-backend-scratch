...
fn epoll_creation(socket: i32) -> i32{
    unsafe{
        ...
        let mut clieve : libc::epoll_event = std::mem::zeroed();
...
fn accepts(socket: i32, sockep: i32) -> i32{
    unsafe{
        ...
        let status_nb = enable_nonblocking(accept_fd);
        if status_nb{
...
fn readandwrite(epollfd : i32, efd : i32) -> i32{
    unsafe{
        ...
        if status < 0{
            ...
            if write_status < 0{
                println!("Error : Write Error - {:?}",std::io::Error::last_os_error().raw_os_error());
                return -1;
            }else{
                println!("Connection Served!! {}", status);
                return 1;
            }
        }
    }
...
fn main(){
    ...
    if status{
        ...
        if bind_status{
            ...
            if listen_status {
                println!("Server Active: Listening on localhost:8080");
